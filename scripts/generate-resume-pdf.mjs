...
function job(entry, indices) {
  ...
  doc.y = y + 15
  body(
    `${entry.title}${entry.isIndependent ? ' · Nights & weekends' : entry.isContract ? ' · Contract' : ''}${entry.companyStatus ? ` · ${entry.companyStatus.label}${entry.companyStatus.date ? ` (${entry.companyStatus.date})` : ''}` : ''}`,
    { lineGap: 0 },
  )
  doc.y += 4
